...
2. **Then use symbol-level retrieval**
   - Prefer `get_repo_outline`, `search_symbols`, `get_symbol` / `get_symbols`, `get_file_outline` (with minimal `context_lines`) over opening whole files.
   - Use `search_text` only when you need non-symbol content (strings, comments, config).

3. **Other MCP and fallbacks**
   - Use **filesystem** MCP for raw file/dir content when symbol tools don’t apply.
   - For Luau-heavy repos, keep using `scripts/luau_symbol_map.sh` and `reports/luau-symbols.tsv` / `reports/luau-modules.tsv` as supplements; still run `index_folder` for the repo so JS/TS/Python are indexed.
   - Run `repo_mapper` when available for entrypoint/dependency mapping after indexing.
   - Treat typo variants like `jmunchcode` / `munchcode` / `jmucnhcode` as: run jCodeMunch `index_folder` (and `index_repo` if applicable), then use symbol tools.

4. **External skill libraries**
   - For MCP-indexed skill libraries (Anthropic skills, Superpowers, planning-with-files, prompt-generator, awesome subagents), see `docs/EXTERNAL-SKILLS-OPENCLAW.md`.

5. **Agents and swarms as extensions**
   - Treat Mission Control agents, subagents, and swarms as **extensions of yourself**. For any task, use the system: run an agent’s `primary_command` from `config/mission-control-agents.json`, submit a goal via POST `/api/goal`, or run a dashboard action. See `docs/AGENT-SYSTEM-AS-EXTENSION.md` and `.cursor/rules/agent-system-as-extension.mdc`.
