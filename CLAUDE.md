<!-- OPENCLAW OPERATIONAL INTELLIGENCE START -->
# OpenClaw — Operational Context (Week 7+)

> This section is maintained by the CEO agent and updated weekly.
> It gives you the real operational state of the system before you touch anything.
> Read this before reading any other file. It saves hours.

## System State (as of 2026-03-01, Week 7)

**Infrastructure:**
- PM2 uptime: 88% (74 up / 10 down). Target >99.5%. Still 12pp short.
- Restart rate: ~8/day (24h rolling window via `pm2 jlist`). Target <5/day.
- Gateway DISCORD_BOT_TOKEN loop: **FIXED** (week 4). Do not escalate this.
- Ollama port conflict: **FIXED** (week 4). 0.0.0.0:11434 is clean.
- Dispatcher pool: max bumped to 20, zero pool errors since week 5.
- Security critical: 3 open (down from 14 worst-case). In sprint, tracked in Greptile.
- Greptile PR blocking: LIVE. Zero overrides in 14 days. Do not bypass it.

**Revenue:**
- AutoPayAgent Stripe: LIVE (integration complete — 16 payment methods, recurring billing capable). No paying customers yet; $0 MRR. Stripe is source of truth for actual revenue.
- SkynPatch: 340 enriched leads/week, 12 sequences active. No calls booked, no reply conversions; link-to-buy clicks tracked separately.
- BWS: 680 leads, 4 cities, 28% open rate.
- CopyLab: first delivery invoiced. Second client onboarding.
- Agency OS: agent-sourced lead; no discovery call booked, no reply conversions (link-to-buy tracked separately).
- QuantFusion: paper-only. Week 8 of paper trading. Sharpe > 1.2.

**Agent Intelligence:**
- Mission control: 28 tasks/day (restored week 4 after 28-day outage).
- Memory cycles: 47 completed. All agents have SOUL.md + MEMORY.md.
- Self-awareness index: 52 repos tracked.
- QA autofix: 72% regression catch rate.
- Model routing: tuned twice. See routing policy below.

## Known Landmines (learned the hard way)

**Don't do these — they caused incidents:**

1. **Do not write history/state to files from concurrent agents.** Use PostgreSQL.
   The action runner had a TOCTOU race on dashboard-action-history.json. Fixed in architect-api.js
   but the pattern is infectious — any new agent doing read-modify-write on a JSON file will
   recreate it.

2. **Do not use `execSync` for PM2 calls.** It blocks the Node event loop. All PM2 queries
   in architect-api.js now go through async exec. Do not reintroduce execSync anywhere on hot paths.

3. **Do not declare `const` inside a try block if you need it outside.** Classic JS scope bug.
   The `taskPlan` variable in handlePostGoal caused `/api/goal` to always crash. Use `let` before
   the try block. This cost a week of silent failures.

4. **Do not burn a one-time token before validating what it's for.** The plan approval UPDATE
   was consuming tokens before cross-checking plan_id. Always add the cross-check to the WHERE
   clause, not as a post-query check.

5. **Do not open file descriptors with `fs.openSync` and forget to close them in the parent**
   after `spawn`. The child inherits them; the parent leaks 2 fds per action invocation.
   Under load this hits the OS fd limit fast.

6. **Do not hardcode action IDs in endpoint handlers.** The research-copy/regenerate endpoint was
   firing CI actions instead of research actions because the IDs were hardcoded wrong. Always
   reference DASHBOARD_ACTIONS by ID, never inline.

7. **Do not duplicate constants.** `freshnessSla` was defined twice in architect-api.js at lines
   ~1119 and ~1355. When one was updated, the other wasn't. Module-level constants only.

## Model Routing Policy (tuned 2x, saving ~$180/mo)

| Job | Model | Notes |
|-----|-------|-------|
| Classification, routing, triage | claude-haiku-4-5 | Fast and sufficient |
| Writing, reasoning, code | claude-sonnet-4-5 | Default for most work |
| Architecture, synthesis, strategic | claude-opus-4-5 | Reserve for high-stakes only |
| Security council synthesis | claude-opus-4-5 | Nightly only, worth it |
| Copy scoring / QA gate | claude-haiku-4-5 | Binary pass/fail, Haiku wins |

Default to Sonnet. Escalate to Opus only when quality is genuinely insufficient.
Never default to Opus out of habit — it's 15x Haiku per token.

## Architecture Conventions (week 7 standards)

- **PostgreSQL** for all persistent state, event logs, approvals, task history
- **Redis** for queue (BullMQ), ephemeral counters, rate limiting
- **PM2** for process management — always query via `pm2 jlist` (async), never `pm2 list` (sync)
- **REPORTS_DIR** = `path.join(__dirname, '..', 'reports')` — project root, not `scripts/reports/`
- **FRESHNESS_SLA** = module-level constant in architect-api.js — don't inline it again
- **DASHBOARD_ACTIONS** = the authoritative list of runbook actions — add new ones there first
- **Stripe**: always use `automatic_payment_methods: { enabled: true }` — not hardcoded method list
- **Greptile**: required before any PR merges. `npm run greptile:scan` locally before pushing.

**Skyn Patch site (skynpatch.com):**
- **Repo:** `v0-skyn-patch` (e.g. `claw-repos/v0-skyn-patch`). Git repo access = v0skynpatch.
- **Deploy:** Push to `main` → Vercel auto-builds and deploys. No separate release step.

## Agent notes — Email provider migration (Resend)

**Git repo system and SaaS:** Target is to switch to **Resend** and retire MailerSend and Maileroo.

- **One Resend account per site.** For each site (e.g. Skyn Patch, BWS, CopyLab): create a separate Resend account, verify the site’s domain, create API key, create webhook, and obtain webhook signing secret.
- **Per-site setup (before any code change):** (1) Resend account, (2) domain verification, (3) API key, (4) webhook URL (e.g. `https://<site>/api/webhooks/resend`), (5) webhook secret for signature verification.
- **Do not replace MailerSend or Maileroo in code until** that site’s Resend account exists and the following are created and added to that site’s env: **API key** and **webhook secret**. Remake sending and webhook handling to use Resend only after credentials are in place.
- **Env per site:** `RESEND_API_KEY`, `RESEND_WEBHOOK_SECRET`, and `EMAIL_PROVIDER=resend` (or equivalent). See `docs/EMAIL_RESEND_MIGRATION.md` for the per-site checklist.

## File Map (things you'll actually need)

```
scripts/architect-api.js    — Mission Control HTTP API server (2016 lines, read in chunks)
scripts/payment-router.js   — Stripe checkout + webhook handler
scripts/bot-commerce.js     — WhatsApp + Discord commerce bot
dashboard/index.html        — Mission Control SPA (998 lines, 9-tab dashboard)
config/agent-team.json      — Agent registry + roles
config/mission-control-agents.json — Mission Control agents (primary_command = how to run each as your extension)
config/external-skills-index.json — jCodeMunch repo ids + best-of entry points for external skills (after npm run skills:install + index)
config/model-routing-policy.json — Model routing rules
docs/AGENT-SYSTEM-AS-EXTENSION.md — Use agents/subagents/swarms as extensions; index then invoke
AGENT_PRINCIPLES.md         — Behavioral contract for all agents (v3.0)
STRATEGY.md                 — Current strategic position (v3.1)
KPIs.md                     — Live metrics, CFO notes (v3.0)
ROADMAP.md                  — What's done, what's active, what's paused (v3.0)
docs/EMAIL_RESEND_MIGRATION.md — Per-site Resend migration (accounts, API, webhooks); do not replace until credentials exist
docs/TOP-PRIORITY-AREAS.md   — Lead gen, PayClaw, ClawPay, Roblox: priority 9, spread across all devices
docs/ROBLOX-PUZZLE-FIGHTER-RESEARCH.md — Super Puzzle Fighter II Turbo mechanics; use so Roblox clone gets it right
config/top-priority-areas.json — Canonical list of top-priority areas
config/repo-completion-master-list.json — Ordered product repos + sections_to_complete for full-completion gap (npm run repo:completion:gap -- --repo <name> | --next)
docs/REPO-FULL-COMPLETION-GAP-RUNBOOK.md — One-repo-at-a-time gap process; capability factory + feature benchmark; best_case_sources
docs/V0-MORNINGOPS-BUILDER-BRIEF.md — v0-morningops Inayan builder brief: any-email support, multiple accounts, features users want; no paywall
docs/SCHEMA-MISMATCH-TOOLS.md — Schema mismatch audit (migrations vs DB, required columns) and comprehensive audit (code vs DB, indexes, FKs)
.env                        — Real env vars (never commit)
.env.example                — Template (safe to read and share)
```

## Quick Health Check Commands

```bash
# PM2 status (async-safe way)
pm2 jlist | node -e "const d=JSON.parse(require('fs').readFileSync('/dev/stdin','utf8')); console.table(d.map(p=>({name:p.name,status:p.pm2_env.status,restarts:p.pm2_env.restart_time})))"

# Queue health
npm run tasks:health

# Security scan
npm run greptile:scan
...
# 3-hour drift heal cycle (manual trigger)
npm run auto:heal:cycle

# Hourly uptime watchdog (find stopped processes, restart, force-run agents)
npm run uptime:watchdog
npm run uptime:watchdog:dry   # dry-run only

# Dashboard health (local + LAN/NAS reachability)
curl -s http://127.0.0.1:4051/api/progress | jq '.ok, .global_status'
curl -s http://$(ipconfig getifaddr en0):4051/api/progress | jq '.ok, .global_status'

# Reports Control Center API smoke checks
curl -s http://127.0.0.1:4051/api/dashboard/reports | jq '.generated_at, (.data.reports | length)'
curl -s http://127.0.0.1:4051/api/dashboard/reports/launch_e2e/history | jq '.data.history | length'

# Stop crash-loop processes (experiment-engine = one-shot CLI; ollama = often not installed on satellite)
pm2 delete claw-experiment-engine 2>/dev/null; pm2 delete m1-laptop-ollama 2>/dev/null; true
# Re-add from config (experiment-engine: cron only; ollama: only on hosts that have Ollama)
pm2 start ecosystem.background.config.js --only claw-experiment-engine
# pm2 start ecosystem.ai-satellite.config.js --only m1-laptop-ollama   # only if Ollama is installed on this host
```

## Known operational behaviors (do not misdiagnose)

- **BUDGET_BLOCKED for LLM triage** — Happens when daily LLM spend caps are exhausted (often late evening). `ANTHROPIC_ALLOWED=false` limits the fallback chain to 3 providers (~$26 combined). Auto-resolves at midnight when budget counters reset. **Only affects the `--analyze` step of daily-fix**; core pipelines (dispatcher, workers, leadgen, etc.) are unaffected. Do not treat as a system outage.
- **Dead letter: copy_lab_run (stale dispatched requeue loop cleanup)** — Reaper-quarantined tasks are classified as cancel-only by reconcile. To **manually requeue** one when convenient: `npm run tasks:reconcile-deadletters -- --requeue-id <task-uuid>`. Example: `npm run tasks:reconcile-deadletters -- --requeue-id b2649444-5784-43e9-9ae7-b5333433e048`.
- **Deep audit FAIL on schema_audit + security_sweep** — The **schema_audit** step in deep audit is separate from the db-level schema check (which can be GREEN). **Security sweep** failures are expected while 3 open criticals are in the Greptile sprint (on track for week 10). Do not block on these for daily operations.
- **Repo_autofix quarantine loop** — Multiple tasks can be lost in DISPATCHED state and quarantined by the reaper after 5 cycles. Reaper behavior is correct. May indicate worker timeout on complex autofix jobs; investigate worker timeout / job complexity if it persists.

## Operational Defaults (new sessions)

1. Verify local dashboard and LAN/NAS accessibility before running fixes.
2. Run one diagnose pass, then one fix pass. Do not loop fix actions without fresh diagnostics.
3. If a fix fails twice in a row, open a blocker with root cause and next action instead of retry spam.
4. Keep Anthropic usage constrained: deterministic/tooling first, then lower-cost model routes, escalate only when needed.

<!-- OPENCLAW OPERATIONAL INTELLIGENCE END -->
