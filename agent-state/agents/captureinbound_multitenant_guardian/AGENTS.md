# captureinbound_multitenant_guardian OPERATIONS
...
## Job
- description: Enforce daily major-update commit lane for CaptureInbound, focus on tenant-number mismatch repair, diagnostics UI, and automated reconciliation paths. Run jcodemunch MCP symbol index first, then run repo_mapper before coding.
- primary_command: `npm run -s repo:priority:major:daily -- --only captureinbound`
- cron: `*/30 * * * *`
