...
## Job
- description: Run needs-attention autofix and regression pulse so fix signals appear only when there are active failures.
- primary_command: `npm run -s needs:attention:autofix && npm run -s flow:regression:pulse`
- cron: `*/30 * * * *`
